...
const DocumentProcessor: React.FC = () => {
    ...
    const handleUpdateDocuments = async () => {
        ...
        try {
            ...
            const payload = suggestions.map((suggestion) => {
                ...
                return {
                    ...rest,
                    suggested_custom_fields: suggested_custom_fields?.filter(
                        (cf) => cf.isSelected,
                    ),
                };
            });
    ...
    const handleCustomFieldSuggestionToggle = (
        docId: number,
        fieldId: number,
    ) => {
        setSuggestions((prevSuggestions) =>
    ...
    const handleCorrespondentChange = (docId: number, correspondent: string) => {
        setSuggestions((prevSuggestions) =>
            prevSuggestions.map((doc) =>
                doc.id === docId
                    ? {...doc, suggested_correspondent: correspondent}
                    : doc,
            ),
        );
    };
    ...
    const handleDocumentTypeChange = (docId: number, documentType: string) => {
        setSuggestions((prevSuggestions) =>
            prevSuggestions.map((doc) =>
                doc.id === docId
                    ? {...doc, suggested_document_type: documentType}
                    : doc,
            ),
        );
    };
    ...
    const handleCreatedDateChange = (docId: number, createdDate: string) => {
        setSuggestions((prevSuggestions) =>
            prevSuggestions.map((doc) =>
                doc.id === docId
                    ? {...doc, suggested_created_date: createdDate}
                    : doc,
            ),
        );
    };
    ...
    const reloadDocuments = async () => {
        ...
        try {
            ...
            setDocuments(data);
            setSelectedDocumentIds(data.map((d: Document) => d.id));
        } catch (err) {
    ...
    // selectOrDeSelectAllDocuments is responsible to select or deselect all documents that are available
    const selectOrDeSelectAllDocuments = async () => {
        switch (selectedDocumentIds.length) {
            ...
            default:
                documents.map((document) => setSelectedDocumentIds(prev => {
            ...
            {documents.length === 0 ? (
                    ...
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex space-x-2">
                            ...
                            <button
                                ...
                                className="bg-blue-600 text-white dark:bg-blue-800 dark:text-gray-200 px-4 py-2 rounded hover:bg-blue-700 dark:hover:bg-blue-900 focus:outline-none"
                            >
                                {processing ? `Processing (${selectedDocumentIds.length})...` : `Generate ${selectedDocumentIds.length <= 1 ? "Suggestion" : "Suggestions"} (${selectedDocumentIds.length})`}
                            </button>
                        </div>
                    </div>
                ...
                Generate Custom Fields
                    ...
                    <div className={"flex w-full justify-end"}>
                        <p className={"w-fit underline text-blue-500 mb-4 cursor-pointer hover:text-blue-600"}
                           onClick={() => selectOrDeSelectAllDocuments()}>{selectedDocumentIds.length == documents.length ? "alle deselectieren" : "alle auswählen"}</p>
                    </div>
                </DocumentsToProcess>
            ) : (
                <SuggestionsReview
